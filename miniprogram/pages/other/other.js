...
Page({
  ...
  reachBottom:util.throttle(function(){
    ...
    if(!searching){
      //非搜索
      let imgNum = that.data.imgInfo_Other.length
      let oldImgInfo = that.data.imgInfo_Other
      wx.showLoading({
        title: '正在获取',
      })

      if(imgNum<100){//限制最大数量
        db.collection('Img').where({category:_.eq(['分类 4'])}).skip(imgNum).limit(10).orderBy('time','desc').get()
        .then(res=>{
          console.log('拉取成功',res)
          let more = res.data
  ...
  onLoad: function (options) {
    ...
    db.collection('Img').where({category:_.eq(['分类 4'])}).limit(12).orderBy('time','desc').get()
    .then(res=>{
      console.log('拉取成功',res)
      this.setData({
        imgInfo_Other:res.data
      })
    }).catch(res=>{
      console.log('拉取失败')
    })
