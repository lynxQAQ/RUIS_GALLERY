...
Page({
  ...
  confirmSearch:function(){
    ...
    if(firstString=='#'){//搜作者
      ...
      ])).orderBy("time", "desc").limit(16).get()
      .then(res=>{
  ...
  reachBottom:util.throttle(function(){
    ...
    if(!searching){
      //非搜索
      let imgNum = that.data.imgInfo_Commercial.length
      let oldImgInfo = that.data.imgInfo_Commercial
      wx.showLoading({
        title: '正在获取',
      })

      if(imgNum<100){//限制最大数量
        db.collection('Img').where({category:_.eq(['分类 2'])}).skip(imgNum).limit(10).orderBy('time','desc').get()
        .then(res=>{
          console.log('拉取成功',res)
          let more = res.data
  ...
  onLoad: function (options) {
    ...
    db.collection('Img').where({category:_.eq(['分类 2'])}).limit(16).orderBy('time','desc').get()
    .then(res=>{
      console.log('拉取成功',res)
      this.setData({
        imgInfo_Commercial:res.data
      })
    }).catch(res=>{
      console.log('拉取失败')
    })
